...
class quiz_session{
  ...
  chooseQuestion(){
    ...
    if (questionIndex < 0) {
      ...
      if (this.questionCount < 0) {
        // infinite - generate more questions
        this.usedQuestions = [];
        console.log("Generatin' 25 new questions, Captain!")
        this.generateQuestions(this.proportions);
        //run this function again with new question set
        this.chooseQuestion();
      }
      else {
  ...
  readResponse(authorId, titleId){
    var responseQuestion,
        responseSource,
        responseAuthor,
        responseTitle;

    responseQuestion = this.currentQuestion.question;

    if(this.answerInput > 1){
      //also quizzing on source
      //element for source
    }
    else{
      responseSource = this.currentQuestion.source;
    }
    ...
    for (var i = 0; i < mc_titles.length; i++) {
      ...
      if(responseNode.classList.contains("active")){
        responseTitle = dict_titles[mc_titles[i]];
        break;
      }
    }
    ...
    for (var i = 0; i < mc_authors.length; i++) {
      ...
      if(responseNode.classList.contains("active")){
        responseAuthor = dict_authors[mc_authors[i]];
        break;
      }
    }
    return new tupleQuestion(responseQuestion,responseSource,responseAuthor,responseTitle);
  }
...
class statisticsTracker {
  ...
  addPoints(points){
    ...
    this.pointsMax += points[1];
    this.percentageTotal = this.percentage(this.points, this.pointsMax);
